...
export async function toggleLike(postId, userId) {
	...
	await runTransaction(db, async tx => {
		...
		if (likeSnap.exists()) {
			tx.delete(likeRef);
			tx.update(postRef, { likesCount: Math.max(currentLikes - 1, 0) });
		} else {
...
export async function toggleSave(postId, userId) {
	...
	await runTransaction(db, async tx => {
		...
		if (saveSnap.exists()) {
			tx.delete(saveRef);
			tx.update(postRef, { savesCount: Math.max(currentSaves - 1, 0) });
		} else {
			tx.set(saveRef, { userId, createdAt: serverTimestamp() });
			tx.update(postRef, { savesCount: currentSaves + 1 });
		}
	});
}
